...
public class SymbolAnalysis {
    ...
    public static String countWord(StringBuffer data) {
        ...
        */
        ...
        for (int i = 0; i < words.length; i++) {
            stringBuilder.append(words[i].charAt(words[i].length() - 1));//удалить последний символ для всх слов к онце предложения
        ...
        res++;
        return "Количество слов в тексте:\t" +res+"\n";
    }
   ...
   public static String longWord(StringBuffer data) {
       ...
       for (int i = 0; i < words.length; i++) {
           ...
           stringBuilder.deleteCharAt(0);
           if (words[i].length() != 0) {
               stringBuilder.append(words[i].charAt(words[i].length() - 1));//удалить предпоследний символ для всех кавычек и скобок
               if (symbols.contains(stringBuilder.toString())){
                   words[i] = words[i].substring(0, words[i].length() -1);
               }
               stringBuilder.deleteCharAt(0);
           }
           if (words[i].length() != 0) {//удалить символ в начале для всех кавычек и скобок
